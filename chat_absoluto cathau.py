...
class ChatWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        ...
        self.last_used_account_display: Optional[str] = None
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QtWidgets.QLabel("Desktop nativo com contas locais, anexos e streaming em tempo real.")
        ...
        control_title = QtWidgets.QLabel("Sessao")
        ...
        control_layout.addWidget(self._labeled_widget("Conta", self.account_combo))
        ...
        control_layout.addWidget(self._labeled_widget("Modelo", self.model_combo))
        ...
        helper_title = QtWidgets.QLabel("Como usar")
        helper_title.setObjectName("sectionTitle")
        helper_layout.addWidget(helper_title)

        helper_body = QtWidgets.QLabel(
            "Escolha uma conta ou deixe em aleatoria.\n"
            "Anexe imagens, texto ou codigo.\n"
            "Enter envia e Shift+Enter quebra linha.\n"
            "Interromper cancela a stream atual."
        )
        helper_body.setObjectName("helperBody")
        ...
        self.chat_title_label = QtWidgets.QLabel("Nova conversa")
        self.chat_title_label.setObjectName("chatHeaderTitle")
        top_layout.addWidget(self.chat_title_label)

        self.chat_subtitle_label = QtWidgets.QLabel("Escreva uma mensagem para iniciar. O app responde em streaming e pode usar a conta aleatoria ou a selecionada.")
        self.chat_subtitle_label.setObjectName("chatHeaderSubtitle")
        self.chat_subtitle_label.setWordWrap(True)
        top_layout.addWidget(self.chat_subtitle_label)
        ...
        badges_row.addWidget(self.mode_badge)

        self.model_badge = QtWidgets.QLabel("Modelo: gpt-5.4")
        self.model_badge.setObjectName("statusBadge")
        badges_row.addWidget(self.model_badge)

        self.account_badge = QtWidgets.QLabel("Conta selecionada: aleatoria")
        ...
        badges_row.addWidget(self.account_badge, 1)
        ...
        chat_frame_layout.setContentsMargins(8, 8, 8, 8)
        ...
        composer_head = QtWidgets.QHBoxLayout()
        composer_head.setSpacing(8)
        composer_title = QtWidgets.QLabel("Mensagem")
        composer_title.setObjectName("sectionTitle")
        composer_head.addWidget(composer_title)
        composer_head.addStretch(1)
        composer_hint = QtWidgets.QLabel("Cole imagem, arraste arquivo ou envie so texto.")
        composer_hint.setObjectName("helperInline")
        composer_head.addWidget(composer_hint)
        composer_layout.addLayout(composer_head)
        ...
        self.prompt_edit.setMaximumHeight(230)
        ...
        self.attach_button.setMinimumSize(150, 46)
        self.attach_button.clicked.connect(self.pick_files)
        action_column.addWidget(self.attach_button)

        self.send_button = QtWidgets.QPushButton("Enviar")
        self.send_button.setObjectName("sendButton")
        self.send_button.setMinimumSize(150, 54)
        ...
        main_layout.addWidget(composer_frame, 0)
    ...
    def _apply_styles(self) -> None:
        self.setStyleSheet(
            ...
            #topBar, #chatFrame, #composerFrame, #attachmentsFrame, #brandCard, #controlCard, #helperCard {
                background: #121820;
                border: 1px solid #252d38;
                border-radius: 18px;
            }
            ...
            #helperBody, #helperInline, #chatHeaderSubtitle {
                color: #8f9baa;
                font-size: 9.5pt;
            }
            #chatHeaderTitle {
    ...
    def current_selected_account_label(self) -> str:
        selected_id = self.account_combo.currentData()
        if selected_id == RANDOM_ACCOUNT_ID or selected_id is None:
            return "aleatoria"
        text = self.account_combo.currentText().strip()
        return ellipsis(text or "sem conta", 64)

    def refresh_header_state(self) -> None:
        model = self.model_combo.currentText().strip() or DEFAULT_MODELS[0]
        self.model_badge.setText(f"Modelo: {model}")

        if self.last_used_account_display:
            account_text = f"Conta usada: {ellipsis(self.last_used_account_display, 84)}"
        else:
            account_text = f"Conta selecionada: {self.current_selected_account_label()}"
        self.account_badge.setText(account_text)

    def on_account_selection_changed(self, _index: int = 0) -> None:
        self.last_used_account_display = None
        self.refresh_header_state()

    def on_model_selection_changed(self, _text: str) -> None:
        self.refresh_header_state()
    ...
    def start_new_chat(self, initial: bool = False) -> None:
        ...
        intro = (
            "Converse com o Codex sem navegador.\n\n"
            "Escolha uma conta especifica ou deixe em aleatoria.\n"
            "Anexe imagens, codigo ou texto como contexto.\n"
            "Use Enter para enviar e Shift+Enter para quebrar linha.\n"
            "O botao Interromper cancela a stream atual."
        )
        self.add_message_widget("welcome", intro, [], "Chat Absoluto | pronto")
        self.mode_badge.setText("Contexto: pronto")
        self.chat_title_label.setText("Nova conversa")
        self.chat_subtitle_label.setText("Tudo pronto para uma nova sessao. Escolha a conta e mande a primeira mensagem.")
        self.refresh_header_state()
        if initial:
    ...
    def send_message(self) -> None:
        ...
        self.set_busy(True)
        self.chat_title_label.setText("Conversando")
        self.chat_subtitle_label.setText("Resposta em andamento. O texto aparece em streaming assim que chega do Codex.")
    ...
    def on_worker_completed(self, result: dict[str, Any]) -> None:
        ...
        self.last_used_account_display = result.get("account_display", "-")
        self.refresh_header_state()
        ...
        if interrupted:
            self.chat_subtitle_label.setText("Geracao interrompida. A conversa continua aberta para voce enviar outra mensagem.")
            self.status_bar.showMessage("Geracao interrompida pelo usuario.")
        elif error:
            self.chat_subtitle_label.setText("A ultima resposta falhou. Ajuste a mensagem ou os anexos e tente novamente.")
            self.status_bar.showMessage(f"Falha: {error}")
        else:
            self.chat_subtitle_label.setText("Resposta concluida. Voce pode continuar a conversa na mesma janela.")
