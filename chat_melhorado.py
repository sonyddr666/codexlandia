...
TEXT_LIKE_EXTENSIONS = {
    ...
    ".ini", ".cfg", ".conf", ".env", ".csv", ".tsv", ".xml", ".html", ".css",
    ".js", ".jsx", ".ts", ".tsx", ".py", ".java", ".kt", ".c", ".cpp", ".cc",
    ".h", ".hpp", ".cs", ".go", ".rs", ".php", ".rb", ".swift", ".sql", ".sh",
    ".bat", ".ps1", ".dockerfile", ".gitignore", ".log", ".diff", ".patch",
}
LANGUAGE_BY_EXTENSION = {
    ".py": "python", ".js": "javascript", ".ts": "typescript", ".tsx": "tsx",
    ".jsx": "jsx", ".json": "json", ".md": "markdown", ".html": "html",
    ".css": "css", ".xml": "xml", ".yml": "yaml", ".yaml": "yaml",
    ".toml": "toml", ".sh": "bash", ".ps1": "powershell", ".sql": "sql",
...
C_SURFACE     = "#0E1420"        # card / panel surface
C_SURFACE2    = "#131928"        # slightly lighter surface
C_BORDER      = "#1E2840"        # subtle border color
C_BORDER_ACT  = "#3B82F6"        # active / focused border
C_ACCENT      = "#3B82F6"        # primary accent (vivid blue)
C_ACCENT2     = "#8B5CF6"        # secondary accent (violet)
C_GREEN       = "#22C55E"        # send button / success
C_GREEN_HOV   = "#16A34A"
C_RED         = "#EF4444"        # stop / error
C_TEXT        = "#E2E8F0"        # primary text
C_TEXT_DIM    = "#64748B"        # muted / secondary text
C_TEXT_MUTED  = "#94A3B8"        # very muted
C_USER_BG     = "#1E3A5F"        # user bubble background
C_USER_BDR    = "#2563EB"        # user bubble border
C_ASST_BG     = "#0E1C30"        # assistant bubble background
C_ASST_BDR    = "#1E3251"        # assistant bubble border
...
class AttachmentData:
    ...
    def to_dict(self) -> dict:
        d = {
            ...
            "text_content": self.text_content, "source_path": self.source_path,
            "truncated": self.truncated, "note": self.note
        }
        if self.raw_bytes:
            d["raw_bytes_b64"] = base64.b64encode(self.raw_bytes).decode("ascii")
        return d
    ...
    def summary(self) -> str:
        icon = "🖼" if self.kind == "image" else ("📄" if self.kind == "text" else "📦")
...
class ConversationMessage:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ConversationMessage":
        msg = cls(role=data["role"], text=data.get("text", ""), ts=data.get("ts", time.time()))
        for att_data in data.get("attachments", []):
            # rebuild basic AttachmentData without binary contents unless stored
            att = AttachmentData(
                id=att_data.get("id", str(time.time())),
                name=att_data.get("name", "Anexo"),
                kind=att_data.get("kind", "binary"),
                mime_type=att_data.get("mime_type", "application/octet-stream"),
                size_bytes=att_data.get("size_bytes", 0),
                text_content=att_data.get("text_content"),
                source_path=att_data.get("source_path"),
                truncated=att_data.get("truncated", False),
                note=att_data.get("note", "")
            )
            if att_data.get("raw_bytes_b64"):
                try:
                    att.raw_bytes = base64.b64decode(att_data["raw_bytes_b64"])
                except Exception:
                    pass
    ...
    def to_api_input(self) -> dict[str, Any]:
        ...
        if self.text.strip():
            parts.append({"type": "text", "text": self.text.strip()})
...
def prepare_text_attachment(name: str, raw_bytes: bytes, source_path: Optional[str] = None) -> AttachmentData:
    ...
    return AttachmentData(
        ...
        name=name, kind="text", mime_type="text/plain",
        size_bytes=original_size, text_content=text,
        source_path=source_path, truncated=truncated, note=note,
    )
...
def prepare_attachment_from_path(path: Path) -> AttachmentData:
    ...
    if mime_type.startswith("image/"):
        ...
        return AttachmentData(
            id=f"att-{time.time_ns()}-{random.randint(1000, 9999)}",
            name=path.name, kind="image", mime_type=mime_type,
            size_bytes=size_bytes, raw_bytes=path.read_bytes(), source_path=str(path),
        )
    if path.suffix.lower() == ".pdf":
        ...
        return AttachmentData(
            id=f"att-{time.time_ns()}-{random.randint(1000, 9999)}",
            name=path.name, kind="binary", mime_type=mime_type, size_bytes=size_bytes,
...
def prepare_attachment_from_qimage(image: QtGui.QImage) -> AttachmentData:
    ...
    return AttachmentData(
        ...
        size_bytes=len(raw), raw_bytes=raw, source_path=None,
    )
...
class TypingIndicator(QtWidgets.QWidget):
    """Three pulsing dots shown while the assistant is generating."""
    ...
    def paintEvent(self, _event: Any) -> None:
        ...
        for i in range(3):
            alpha = 220 if i == self._phase else 80
            color = QColor(C_ACCENT)
            color.setAlpha(alpha)
            painter.setBrush(color)
            painter.setPen(Qt.PenStyle.NoPen)
            painter.drawEllipse(x_start + i * gap, y_center - dot_r, dot_r * 2, dot_r * 2)
...
class MessageBubble(QtWidgets.QFrame):
    def __init__(
        self, role: str, header_text: str, text: str = "",
        ...
        self.copy_button.setCursor(QtGui.QCursor(Qt.CursorShape.PointingHandCursor))
        ...
        # ── typing indicator (only while streaming) ───────────
    ...
    def _copy_text(self) -> None:
        if self._text:
            ...
            QTimer.singleShot(1800, lambda: self.copy_button.setText("Copiar"))
...
class PendingAttachmentRow(QtWidgets.QFrame):
    ...
    def __init__(self, attachment: AttachmentData, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        if attachment.kind == "image" and attachment.raw_bytes:
            ...
            thumb.setFixedSize(QSize(48, 48))
            pixmap = QtGui.QPixmap()
            pixmap.loadFromData(attachment.raw_bytes)
            thumb.setPixmap(
                pixmap.scaled(48, 48,
        ...
        remove_button.setFixedSize(QSize(28, 28))
        remove_button.setCursor(QtGui.QCursor(Qt.CursorShape.PointingHandCursor))
        remove_button.setToolTip("Remover anexo")
        remove_button.setStyleSheet(
            "QPushButton { background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.30);"
            "  border-radius: 6px; color: #EF4444; font-weight: 700; font-size: 10pt; }"
...
class ChatWorker(QThread):
    ...
    def run(self) -> None:
        result = {
            ...
            "account_id": self.account.id, "account_display": self.account.display_name,
            "model": self.model,
        }
        try:
            ...
            if self.store_preference is not False:
                attempts.append({
                    ...
                    "payload": self._build_payload(
                        ...
                        if (same_account and self.previous_response_id) else None,
                    ),
                })
            ...
            for attempt in attempts:
                ...
                if attempt["name"] == "native" and self._should_fallback(response.status_code, body):
                    result["store_supported"] = False
                    self.statusChanged.emit("Conta rejeitou store ou previous_response_id. Caindo para modo stateless...")
                    continue
    ...
    def _post_stream(self, payload: dict[str, Any]) -> requests.Response:
        try:
            response = requests.post(
                CODEX_URL, json=payload, headers=self.account.auth_headers(),
    ...
    def _consume_response(self, response: requests.Response, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            payload_obj = raw_text
        text = extract_non_stream_text(payload_obj)
        if text:
            result["text"] = text
...
class ChatWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        sidebar.setObjectName("topBar") # reuse topBar styling for rounded corners & bg
        ...
        title = QtWidgets.QLabel("✦ Chat Absoluto")
        title.setObjectName("appTitle")
        logo_col.addWidget(title)
        subtitle = QtWidgets.QLabel("Codex API Client Melhorado")
        subtitle.setStyleSheet(f"color: {C_TEXT_DIM}; font-size: 8pt;")
        logo_col.addWidget(subtitle)
        sidebar_layout.addLayout(logo_col)

        sidebar_layout.addWidget(self._vline())

        # Account picker
        self.account_combo = QtWidgets.QComboBox()
        self.account_combo.setMinimumHeight(38)
        sidebar_layout.addWidget(self._labeled_widget("CONTA", self.account_combo))
        ...
        sidebar_layout.addWidget(self.refresh_accounts_button)
        ...
        sidebar_layout.addWidget(self._labeled_widget("MODELO", self.model_combo))
        ...
        # Status badges
        badges_col = QtWidgets.QVBoxLayout()
        badges_col.setSpacing(8)
        self.mode_badge = QtWidgets.QLabel("● Contexto: pronto")
        self.mode_badge.setObjectName("modeBadge")
        badges_col.addWidget(self.mode_badge)
        self.account_badge = QtWidgets.QLabel("Conta: —")
        self.account_badge.setObjectName("accountBadge")
        self.account_badge.setWordWrap(True)
        badges_col.addWidget(self.account_badge)
        sidebar_layout.addLayout(badges_col)
        ...
        self.attach_button = QtWidgets.QPushButton("📎 Anexos")
        self.attach_button.setObjectName("attachButton")
        self.attach_button.setFixedSize(100, 42)
        ...
        self.prompt_edit.setPlaceholderText(
            "Escreva sua mensagem...   Enter envia  ·  Shift+Enter quebra linha  ·  Arraste arquivos ou cole imagens"
        )
        ...
        self.send_button.setFixedSize(110, 42)
        self.send_button.clicked.connect(self.send_message)
        input_row.addWidget(self.send_button)
        composer_layout.addLayout(input_row)

        # Hint line
        hint = QtWidgets.QLabel("Dica: cole imagens com Ctrl+V · Arraste arquivos direto na caixa de texto")
        hint.setStyleSheet(f"color: {C_TEXT_DIM}; font-size: 8pt;")
        ...
        self.status_bar.showMessage("Pronto. Coloque um auth*.json na pasta para autenticar.")
    def _vline(self) -> QtWidgets.QFrame:
    ...
    def load_accounts_into_selector(self, show_status: bool = True) -> None:
        ...
        self.account_combo.addItem("🎲  Aleatória", RANDOM_ACCOUNT_ID)
        ...
        if show_status:
            if self.accounts:
                self.status_bar.showMessage(f"{len(self.accounts)} conta(s) detectada(s).")
            else:
                self.status_bar.showMessage("Nenhuma conta detectada. Coloque um auth*.json válido nesta pasta.")
        ...
        try:
            ...
            if data and isinstance(data, list):
                ...
                for msg_data in data:
                    ...
                    role_display = "Eu" if msg.role == "user" else "AI"
                    border = C_USER_BDR if msg.role == "user" else C_ASST_BDR
                    header = f"Você  ·  {human_time(msg.ts)}" if msg.role == "user" else f"✦ Codex  ·  {human_time(msg.ts)}"
                ...
                self.status_bar.showMessage("Historico carregado com sucesso.")
        except Exception as e:
    ...
    def start_new_chat(self, initial: bool = False) -> None:
        if self.current_worker is not None:
            QtWidgets.QMessageBox.information(
                self, APP_NAME, "Interrompa a geração atual antes de iniciar uma nova conversa."
            )
            return

        if initial and HISTORY_FILE.exists():
            self.load_history()
            return
        ...
        intro = (
            "Olá! Sou o assistente Codex. Como posso te ajudar hoje?\n\n"
            "  • Escolha uma conta ou deixe em modo aleatório\n"
            "  • Selecione o modelo desejado\n"
            "  • Anexe imagens, código ou arquivos de texto\n"
            "  • Enter envia · Shift+Enter quebra linha\n"
            "  • O botão Parar cancela a stream a qualquer momento"
        )
        self.add_message_widget("assistant", intro, [], "✦ Codex  ·  pronto")
        self.mode_badge.setText("● Contexto: pronto")
        self.account_badge.setText("Conta: —")
        self.status_bar.showMessage("Nova conversa iniciada." if not initial else "Pronto.")
    ...
    def add_message_widget(
        ...
        if role == "user":
            # right-align with avatar
            avatar = self._make_avatar("Eu", C_USER_BDR)
            row_layout.addStretch(1)
            row_layout.addWidget(bubble)
            row_layout.addSpacing(8)
            row_layout.addWidget(avatar, 0, Qt.AlignmentFlag.AlignTop)
        else:
            # left-align with avatar
            avatar = self._make_avatar("AI", C_ASST_BDR)
            row_layout.addWidget(avatar, 0, Qt.AlignmentFlag.AlignTop)
            row_layout.addSpacing(8)
            row_layout.addWidget(bubble)
            row_layout.addStretch(1)
    ...
    def _make_avatar(self, text: str, border_color: str) -> QtWidgets.QLabel:
        avatar = QtWidgets.QLabel(text)
        avatar.setFixedSize(QSize(32, 32))
        avatar.setAlignment(Qt.AlignmentFlag.AlignCenter)
        avatar.setStyleSheet(
            f"background: {C_SURFACE2}; border: 1px solid {border_color};"
            f"border-radius: 16px; color: {C_TEXT_MUTED}; font-size: 8pt; font-weight: 700;"
        )
        return avatar
    ...
    def refresh_attachment_widgets(self) -> None:
        ...
        label = QtWidgets.QLabel(f"📎  {len(self.pending_attachments)} anexo(s) prontos para envio")
        label.setStyleSheet(f"color: {C_ACCENT2}; font-size: 8pt; font-weight: 700; padding: 0 4px;")
    ...
    def add_files(self, files: list[str]) -> None:
        ...
        self.refresh_attachment_widgets()
        if added:
            self.status_bar.showMessage(f"{added} anexo(s) preparado(s).")
        if errors:
    ...
    def stop_generation(self) -> None:
        if self.current_worker is None:
            return
        self.status_bar.showMessage("Interrompendo geração...")
    ...
    def send_message(self) -> None:
        ...
        if account is None:
            QtWidgets.QMessageBox.warning(
                self, APP_NAME,
                "Nenhuma conta encontrada. Coloque um auth*.json válido na pasta do programa.",
            )
            return
        ...
        self.add_message_widget(
            "user",
            text or "(mensagem só com anexos)",
            attachments,
            f"Você  ·  {human_time(user_message.ts)}",
        )
        ...
        self.current_assistant_bubble = self.add_message_widget(
            "assistant", "", [], f"✦ Codex  ·  {account.display_name}  ·  {model}"
        )
        self.current_assistant_bubble.show_typing(True)
        self.current_stream_received_text = False
    ...
    def on_worker_delta(self, delta: str) -> None:
        ...
        if not self.current_stream_received_text:
            self.current_assistant_bubble.show_typing(False)
    ...
    def on_worker_completed(self, result: dict[str, Any]) -> None:
        ...
        self.account_badge.setText(f"Conta: {result.get('account_display', '—')}")
        ...
        if final_text:
            ...
            if partial:
                ...
                bubble.set_text("[Geração interrompida]")
        elif error:
            if bubble is not None:
        ...
        if interrupted:
            self.status_bar.showMessage("Geração interrompida pelo usuário.")
        elif error:
            self.status_bar.showMessage(f"Falha: {error}")
        else:
            self.status_bar.showMessage("✓  Resposta concluída.")
...
def main() -> int:
    ...
    # Force a dark palette so native widgets don't fight the stylesheet
    palette = QPalette()
    palette.setColor(QPalette.ColorRole.Window,          QColor(C_BG))
    palette.setColor(QPalette.ColorRole.WindowText,      QColor(C_TEXT))
    palette.setColor(QPalette.ColorRole.Base,            QColor(C_SURFACE2))
