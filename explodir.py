...
class BolsaCodexApp:
    ...
    def setup_ui(self):
        ...
        self.log_area = scrolledtext.ScrolledText(log_frame, height=15, bg="#010409", fg="#a5d6ff", font=("Consolas", 9), borderwidth=1, relief=tk.SUNKEN)
    ...
    def monitoring_loop(self):
        ...
        while self.is_monitoring:
            ...
            self.log(f"A procurar ficheiros JSON... ({len(json_files)} encontrados)")
            
            for f in json_files:
                try:
                    self.log(f"A processar: {f.name}")
                    ...
                    self.log(f"  {f.name}: {len(entries)} contas encontradas")
                        
                    for entry in entries:
                        res = self.process_account(entry, f.name, data)
                        if res: all_results.append(res)
                except Exception as e:
                    self.log(f"  ERRO em {f.name}: {e}")
            
            self.log(f"Resultados: {len(all_results)} contas processadas")
    ...
    def process_account(self, entry, filename, data):
        ...
        self.log(f"A verificar API para: {r['email']} (exp: {jwt_exp(tok)})")
    ...
    def save_new_account(self, tokens):
        ...
        email_name = email.split('@')[0] if '@' in email else f"Injetada_{datetime.now().strftime('%H%M')}"
            ...
            try:
                with open(filename, 'r', encoding='utf-8') as f:
                    ...
                    for i, acc in enumerate(pool):
                        if acc.get("account_id") == acc_id or jwt_email(acc.get("access_token", "")) == email:
                            pool[i] = new_entry
                            with open(filename, 'w', encoding='utf-8') as f:
                                json.dump(data, f, indent=2)
                            self.log(f"CONTA ATUALIZADA: {email_name}")
                            return
            except: pass
