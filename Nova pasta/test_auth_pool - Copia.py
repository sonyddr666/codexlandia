...
# --- SISTEMA DE LOG E MEMÓRIA ---
live_logs = []
account_history = {} # Guarda o estado do último tick
...
def parse_quota(body):
    ...
    def hunt_quotas(obj):
        if isinstance(obj, dict):
            ...
            # Matemática da API troll: se vier used_percent, a gente inverte
...
def test_and_renew(provider, entry, filename, data):
    ...
    r = dict(label=label, provider=provider, email='-', token_exp='-', quota={}, http_status=None, result='?', error='', diff_5h='', diff_sem='', diff_http='')
    ...
    if needs_renew:
        ...
        if new_auth and 'access_token' in new_auth:
            ...
            if status == "DELETADO":
                ...
                add_log(f"{R}MORTO:{X} {r['email']} ({filename}) token morreu e o arquivo foi pro ralo.")
            else:
    ...
    if uid in account_history:
        ...
        if prev['sem'] is not None and curr_sem is not None:
            if curr_sem < prev['sem']:
                r['diff_sem'] = f" {R}↓{X}"
                add_log(f"{M}CONSUMO SEMANAL:{X} {r['email']} gastou: {prev['sem']}% -> {curr_sem}%")
            elif curr_sem > prev['sem']:
                r['diff_sem'] = f" {G}↑{X}"

    account_history[uid] = {'5h': curr_5h, 'sem': curr_sem, 'http': curr_http}
    return r

def color_pct_diff(pct, diff_arrow):
    # O espaço vazio alinha a tabela perfeitamente quando não tem seta
...
def render(all_results, time_left):
    ...
    for provider, results in all_results.items():
        ...
        print(B+f'── {provider.upper()} ({len(results)} conta(s)) ' + '─'*68 + X)
        print(f"  {'#':<3} {'label':<34} {'resultado':<20} {'HTTP':>10} {'5h%':>12} {'reset5h':>9}  {'sem%':>12} {'resetSem':>10}  {'email':<28}")
        print(f"  {'-'*3} {'-'*34} {'-'*20} {'-'*10} {'-'*12} {'-'*9}  {'-'*12} {'-'*10}  {'-'*28}")
        ...
        for i, r in enumerate(results, 1):
            ...
            print(f"  {i:<3} {r['label'][:33]:<34} {r['result']:<20} {http_str:>10} "
                  f"{c_5h:>21} {fmt_remaining(fh_rst):>9}  {c_sem:>21} {fmt_reset_abs(wk_rst)[:10]:>10}  "
                  f"{r['email'][:27]:<28}")
            
        print()
        
    print(C+B+'── LIVE LOGS ' + '─'*83 + X)
    if not live_logs:
        print(f"  {GR}Nenhuma alteração detectada ainda... Aguardando.{X}")
    else:
...
def main():
    ...
    add_log(f"{C}SISTEMA INICIADO:{X} Monitoramento ativo por 5 minutos.")
